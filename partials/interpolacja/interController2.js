app.controller('InterExampleController', function($scope, $state, $menuHide){
    $scope.$emit("sideCont", [
        {
            name: "Co to jest interpolacja ?", 
            click: function(){
        ...
        {
            name: "Prezentacja działania metody",
            click: function(){
                if($("#bprezentacjadzialaniametody").hasClass("sidebSelected"))
                    return;    
                $('#contCol').animate({
                    scrollTop: $("#prezentacjadzialaniametody").offset().top
                }, 1000);
              
            }, 
            showCircle: true,
            contentID: "prezentacjadzialaniametody"
        },
        {
            name: "Powrót",
            click: function(){
    ...
    $menuHide.onHide(function(){
        ...
        brd2.needsFullUpdate = true;
        brd2.resizeContainer($("#g2well").width(), $("#g2well").height());
        brd2.update();
